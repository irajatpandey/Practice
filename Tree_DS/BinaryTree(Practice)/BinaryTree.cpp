...
void printNonLeaf(BinaryTree<int> *root){
    ...
    if(root -> left != NULL or root -> right != NULL)
        cout << root -> data << " ";
    printNonLeaf(root -> left);      // LeftSubtree 
    printNonLeaf(root -> right);    // RightSubTree
}
void printLeaf(BinaryTree<int> *root){
    if(root == NULL) return; // base condition

    if(root -> left == NULL and root -> right == NULL)
        cout << root -> data << " ";
    printLeaf(root -> left);      // LeftSubtree 
    printLeaf(root -> right);    // RightSubTree
}
