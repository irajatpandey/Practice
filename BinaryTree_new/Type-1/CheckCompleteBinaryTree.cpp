...
int totalNodes(BinaryTree<int> *root){
    if(root == NULL) return 0;
    return 1 + totalNodes(root -> left) + totalNodes(root -> right);
}
bool checkComplete(BinaryTree<int> *root, int index, int numberOfNodes){
    if(root == NULL) return true;
    if(index >= numberOfNodes)
        return false;
    return checkComplete(root -> left, 2 * index + 1, numberOfNodes) and checkComplete(root -> right, 2 * index + 2, numberOfNodes);    
