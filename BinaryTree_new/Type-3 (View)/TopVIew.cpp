...
void printTopView(BinaryTree<int> *root, map<int, pair<int,int>> &map, int level, int dist ){
    if(root == NULL) return;
    if(map.find(dist) == map.end() or map[dist].first  > level)
        map[dist].second = root -> data;
...
void topView(BinaryTree<int> *root){
    ...
    printTopView(root, map, level, dist);
